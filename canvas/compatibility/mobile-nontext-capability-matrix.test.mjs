...
test("non-typography mobile matrix rows have explicit final verdicts", () => {
  for (const target of ["swift", "kotlin"]) {
    ...
    assert.deepEqual(unresolved, [], `${target} retained an unverified non-typography row`);
    for (const path of accounted) assert.ok(
      ["native", "raster", "ignore"].includes(capabilityTableFor(target).properties[path].verdict),
      `${target}:${path} has no final verdict`,
    );
  }
});
