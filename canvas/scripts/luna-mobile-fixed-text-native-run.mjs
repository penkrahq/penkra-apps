...
function parseIosRoot(caseID, nonce, log) {
  const n = "(-?(?:\\d+(?:\\.\\d*)?|\\.\\d+))";
  const match = String(log).match(new RegExp(`LUNA_FIXED_TEXT_ROOT case=${escapeRegex(caseID)} nonce=${escapeRegex(nonce)} frame=${n},${n} ${n}x${n} scale=${n}`, "u"));
...
function receipt(log, caseID, nonce, isAndroid) { const escaped = escapeRegex(caseID); const n = escapeRegex(nonce); return new RegExp(`LUNA_FIXED_TEXT_READY case=${escaped} nonce=${n}(?:\\n|$)`, "u").test(log) && (isAndroid ? parseAndroidRoot(caseID, nonce, log) : parseIosRoot(caseID, nonce, log)); }
...
async function runIos() {
  ...
  const builtBinaryRoot = join(derived, "Build/Products/Debug-iphonesimulator/CanvasFixedTextEvidence.app");
  const binaryRoot = resolve(process.env.CANVAS_FIXED_TEXT_IOS_APP_PATH ?? builtBinaryRoot);
  const native = join(evidenceRoot, process.env.CANVAS_FIXED_TEXT_IOS_RUN_ID ?? "native-ios-run-01"); await mkdir(native, { recursive: true });
  let prior = new Map();
  if (process.env.CANVAS_FIXED_TEXT_IOS_ONLY_UNMEASURED === "1") {
    try { const rows = JSON.parse(await readFile(join(evidenceRoot, process.env.CANVAS_FIXED_TEXT_IOS_PRIOR_RUN ?? "native-ios-run-03", "measurements.json"), "utf8")).entries ?? []; prior = new Map(rows.map((row) => [`${row.device}/${row.contentSize}/${row.caseId}`, row])); } catch { /* a missing prior is equivalent to no filter */ }
  }
  if (process.env.CANVAS_FIXED_TEXT_SKIP_IOS_BUILD !== "1") await record("xcodegen", ["generate", "--spec", "project.yml"], { cwd: evidenceRoot });
  try {
    if (process.env.CANVAS_FIXED_TEXT_SKIP_IOS_BUILD !== "1") {
    ...
    } else await save(join(native, "build-result.json"), { skipped: true, reason: "reused previously completed build", appPath: binaryRoot });
      ...
      try {
        for (const contentSize of declared.contentSizes) {
          ...
          for (const item of TEXT_CASES) {
            const previous = prior.get(`${declared.key}/${contentSize}/${item.id}`);
            if (previous && previous.status !== "unmeasured") continue;
            const nativeCaseID = `fixed-text-${item.id}`;
            const nonce = `fixed-text-${declared.key}-${contentSize}-${item.id}-${randomUUID()}`; const start = Date.now();
            await record("xcrun", ["simctl", "launch", "--terminate-running-process", declared.id, IOS_BUNDLE_ID, "--canvas-case", nativeCaseID, "--canvas-nonce", nonce]);
            let log = ""; let ready = null; for (let attempt = 1; attempt <= 20; attempt += 1) { await sleep(500); log = await iosLog(declared, start); ready = receipt(log, nativeCaseID, nonce, false); if (ready) break; }
            ...
            const image = await decode(full[1].bytes); assert.equal(ready.root.width, 340); assert.equal(ready.root.height, 180); const rect = { x: Math.round(ready.root.x * ready.scale), y: Math.round(ready.root.y * ready.scale), width: Math.round(ready.root.width * ready.scale), height: Math.round(ready.root.height * ready.scale) }; const cropped = await crop(full[1].bytes, rect); const capturePath = join(entryDir, "crop.png"); await writeFile(capturePath, cropped.bytes); const reference = await decode(await readFile(join(evidenceRoot, "references/ios", `${declared.key}-${contentSize}`, `${item.id}.png`))); const comparison = compareFixedTextPixels(reference, cropped); const row = { platform: "ios", device: declared.key, contentSize, caseId: item.id, classification: classifyFixedTextCase(item.id), status: comparison.status, measurementStatus: "measured", nonce, readiness: ready, stability, capturePath, comparison, fullDimensions: { width: image.width, height: image.height } }; await save(join(entryDir, "measurement.json"), row); entries.push(row);
          }
        }
...
async function runAndroid() {
  const native = join(evidenceRoot, process.env.CANVAS_FIXED_TEXT_ANDROID_RUN_ID ?? "native-android-run-01"); await mkdir(native, { recursive: true }); const fixture = await mkdtemp(join(tmpdir(), "canvas-fixed-text-compose-")); const sourceDir = join(fixture, "app/src/main/java/generated/canvas"); const packageDir = join(fixture, "app/src/main/java/com/penkra/canvas/fixture");
  await cp(resolve(root, "compatibility/mobile-fixtures/compose"), fixture, { recursive: true });
  for (const file of await (await import("node:fs/promises")).readdir(join(evidenceRoot, "compose/generated"))) if (file !== "MainActivity.kt") await cp(join(evidenceRoot, "compose/generated", file), join(sourceDir, file));
  await cp(join(evidenceRoot, "compose/generated/MainActivity.kt"), join(packageDir, "MainActivity.kt"));
  const adbDevices = await record(adb, ["devices"]); let startedProcess = null; if (!/emulator-5554\s+device/u.test(adbDevices.stdout)) {
    const avds = await optional(emulator, ["-list-avds"]);
    if (!avds.stdout?.trim()) {
      const entries = ANDROID_STATES.flatMap((state) => TEXT_CASES.map((item) => ({ platform: "android", density: state.density, fontScale: state.fontScale, caseId: item.id, classification: classifyFixedTextCase(item.id), status: "unmeasured", measurementStatus: "unmeasured", reason: "no connected emulator and no configured AVD; no native readiness receipt or frame exists" })));
      const apkPath = process.env.CANVAS_FIXED_TEXT_ANDROID_APK_PATH ? resolve(process.env.CANVAS_FIXED_TEXT_ANDROID_APK_PATH) : null;
      if (apkPath) { const apkBytes = await readFile(apkPath); await save(join(native, "binary-facts.json"), { apkPath, apkSha256: sha256(apkBytes), apkBytes: apkBytes.length }); await save(join(native, "build-result.json"), { skipped: true, reason: "reused previously completed valid APK; no configured AVD for capture", apkPath }); }
      await save(join(native, "measurements.json"), { platform: "android", entries, counts: counts(entries), sourceHashes: JSON.parse(await readFile(join(evidenceRoot, "source-hashes.json"))) }); await save(join(native, "device-restoration.json"), { connectedDevice: null, avdList: [], restored: true, reason: "no Android device state was changed" }); await rm(fixture, { recursive: true, force: true }); return;
    }
    startedProcess = spawn(emulator, ["-avd", "penkra_api36_pixel8"], { detached: true, stdio: "ignore" }); startedProcess.unref(); await record(adb, ["wait-for-device"], { timeout: 120000 }); for (let i = 0; i < 60; i += 1) { if ((await optional(adb, ["-s", "emulator-5554", "shell", "getprop", "sys.boot_completed"])).stdout?.trim() === "1") break; await sleep(1000); }
  }
...
const commandRunId = platform === "android" ? (process.env.CANVAS_FIXED_TEXT_ANDROID_RUN_ID ?? "native-android-run-01") : (process.env.CANVAS_FIXED_TEXT_IOS_RUN_ID ?? "native-ios-run-01");
await save(join(evidenceRoot, commandRunId, "commands.json"), commandLog.map(({ stdout, stderr, ...entry }) => entry));
