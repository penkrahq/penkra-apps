...
const evidence = resolve(process.env.CANVAS_GRID_NATIVE_EVIDENCE_ROOT ?? join(sourceEvidence, "native-run-01"));
...
async function waitForReceipt(device, caseID, nonce, launchDir) {
  ...
  for (let attempt = 1; attempt <= 20; attempt += 1) {
    ...
    if (result.code === 0 && readyReceipt(caseID, nonce, combined)) return { status: "ready", attempt, logPath: relativeEvidence(logPath), pattern: `LUNA_GRID_READY case=${caseID} nonce=${nonce}` };
    await sleep(500);
  }
...
function cropRect(image, scale) {
  const width = Math.round(340 * scale);
...
async function captureCase(device, contentSize, caseID, scale, expected, referencePath, stateDir, sequence) {
  const launchDir = join(stateDir, "launches", caseID);
  const nonce = `grid-run-01-${device.key}-${contentSize}-${caseID}-${sequence}`;
  ...
  try { launch = await command("xcrun", args, { timeout: 30000, killSignal: "SIGTERM" }); }
    ...
    return { caseID, deviceId: device.id, contentSize, scale, nonce, launchArguments: args, status: "unmeasured", reason: "simctl launch failed", launchExitCode: error.exitCode, launchFailurePath: relativeEvidence(join(launchDir, "launch-failure.log")), referencePath: relativeEvidence(referencePath) };
  }
  ...
  if (!pid) return { caseID, deviceId: device.id, contentSize, scale, nonce, launchArguments: args, status: "unmeasured", reason: "simctl launch returned no PID", referencePath: relativeEvidence(referencePath) };
  const readiness = await waitForReceipt(device, caseID, nonce, launchDir);
  if (readiness.status !== "ready") return { caseID, deviceId: device.id, contentSize, scale, nonce, launchArguments: args, pid, status: "unmeasured", reason: readiness.reason, readiness, referencePath: relativeEvidence(referencePath) };
  await sleep(2500);
  const fullA = join(stateDir, "full", `${caseID}-a.png`);
  const fullB = join(stateDir, "full", `${caseID}-b.png`);
  ...
  if (!stability.stable) return { caseID, deviceId: device.id, contentSize, scale, nonce, launchArguments: args, pid, status: "unmeasured", reason: "consecutive screenshot SHA-256 hashes were not identical valid hashes", readiness, stability, referencePath: relativeEvidence(referencePath) };
  const crop = cropRect(second.image, scale);
  const capturePath = join(stateDir, `${caseID}.png`);
  await command("sips", ["-c", String(crop.height), String(crop.width), fullB, "--out", capturePath]);
  ...
  return { caseID, deviceId: device.id, contentSize, scale, nonce, launchArguments: args, pid, status, measurementStatus: "measured", readiness, stability, fullCapturePaths: stability.paths, capturePath: relativeEvidence(capturePath), referencePath: relativeEvidence(referencePath), crop, comparison };
}
...
const retainedSourceReceipt = JSON.parse(await readFile(join(sourceEvidence, "source-hashes.json"), "utf8"));
const currentSourceReceipt = sourceHashReceipt(buildGridSources().sources);
if (currentSourceReceipt.sourceSha256 !== retainedSourceReceipt.sourceSha256) throw new Error(`Generated Swift source hash changed: ${currentSourceReceipt.sourceSha256} != ${retainedSourceReceipt.sourceSha256}`);
...
await copyFile(join(sourceEvidence, "source-hashes.json"), join(evidence, "source-hashes.json"));
