...
function mismatchCategories(reference, actual, base, comparison, caseId) {
  const categories = { glyphEdge: 0, decoration: 0, colorInterior: 0, interior: 0, topology: 0 };
  for (const component of comparison.components) for (let y = component.minY; y <= component.maxY; y += 1) for (let x = component.minX; x <= component.maxX; x += 1) {
    ...
    if (DECORATION_CASES.has(caseId) && !baseInk && (refInk || actualInk)) categories.decoration += 1;
...
export async function analyzeRetainedEvidence({ evidenceRoot, referencesRoot, outputRoot = null } = {}) {
  ...
  for (const result of run.results) {
    ...
    const referenceForeground = foregroundStats(reference, manifestEntry.scale); const nativeForeground = foregroundStats(native, manifestEntry.scale); const comparison = comparePixelArrays(reference, native); const categories = mismatchCategories(reference, native, base, comparison, result.caseId);
