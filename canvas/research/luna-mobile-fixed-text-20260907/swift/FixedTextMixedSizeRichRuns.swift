...
public struct FixedTextMixedSizeRichRuns: View {
  ...
  public var body: some View {
    ZStack(alignment: .topLeading) {
      (Text("Small ").font(.custom("Inter", size: 24, relativeTo: .body).weight(.regular)).foregroundColor(Color(red: 0.8, green: 0.333, blue: 0, opacity: 1)) + Text("BIG").font(.custom("Inter", size: 36, relativeTo: .body).weight(.bold)).foregroundColor(Color(red: 0.8, green: 0.333, blue: 0, opacity: 1))).opacity(1).frame(width: 300, height: 64, alignment: .topLeading).position(x: 170, y: 56)
