...
| 55 | **Agent text offsets are UTF-16 code units.** Storage, JavaScript execution, and CanvasKit all use the same indexing model; converting the public mutation API to grapheme indices would introduce a second coordinate system without removing the need for UTF-16 internally. The interactive editor alone keeps grapheme clusters indivisible for caret movement and deletion. |
| 56 | **The agent surface stays small and coherent:** domain operations locate, create, move, share, open, extract, and export documents; `documents.execute` is the one programmable, atomic design read/mutation surface. Splitting every design primitive into another operation would duplicate the document language and weaken atomicity. Discoverability belongs in the installed manual and module Skills. |
| 57 | **Frames carry roles; export requests select targets/formats.** `slide`, `page`, `route`, `ios`, and `android` describe what a frame is. `pptx`, `pdf`, `html`, `swift`, `kotlin`, `png`, and `svg` describe what is emitted. Public operations may infer a target from a role where the mapping is singular, but the concepts are not interchangeable. |
| 58 | **Roleless subtree extraction uses target-oriented IR.** It resolves layout and modes, derives bounds from the requested subtree, applies the SVG capability table, carries no frame role or physical-page sizing, and rasterizes only unsupported isolated descendants. It never borrows a containing frame's role. |
...
| Q14 | ~~Marks are UTF-16 offsets (§21.3). Does the agent-facing API expose UTF-16 or grapheme indices?~~ | Stage 5 | **CLOSED — UTF-16 in storage and the agent API; grapheme-aware caret/editing in the interactive editor (decision 55).** |
...
**Indices are UTF-16**, matching JavaScript string indexing, storage, and the agent-facing mutation
API. Grapheme clusters stay indivisible as caret units, so the interactive editor keeps its own
grapheme layer above those offsets (decision 55).
