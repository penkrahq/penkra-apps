...
the requested SHA-256 and byte length. A returned MIME must match a requested
MIME; an omitted returned MIME remains compatible. Backend path normalization
cannot replace the caller path. Uploading receipts must provide a positive
...
Exit `0`; **54 passed, 0 failed, 0 cancelled, 0 skipped**; duration
`271.344292 ms`. The count includes the four nested phase checks in the
protocol test.

The new five-case suite covers synchronous caller mutation isolation with exact
multipart chunks, valid ready receipts and backend-path normalization, ready
receipt missing/wrong hash and size plus array/MIME failures, equivalent
multipart completion failures, and invalid chunk sizes before the first part.
...
harness, the isolated new suite passed `5/5`, the existing API suite passed
`20/20`, and the final combined selection above passed completely.
