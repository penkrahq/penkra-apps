...
- Replaced-file, replaced-symlink, replaced-directory, mixed-receipt, and earlier-batch replacement
  fixtures now rename the original entry to a distinct retained path inside the same temporary
  root before creating the replacement. This keeps the original inode allocated and makes the
  identity-change assertions deterministic. Production receipt semantics were not changed.
- Added valid `"é".repeat(127) + "a"` (255 UTF-8 bytes) and invalid `"é".repeat(128)` (256
  UTF-8 bytes) rows.
- Every invalid bundle-entry case asserts its destination remains absent.
- Review follow-up command:
  `node --test src/export-bundle.test.mjs src/export-service.test.mjs src/export-publication-matrix.test.mjs`
  — exit 0; 24 passed, 0 failed/cancelled/skipped, 4.89 seconds.
- Review follow-up log: `/tmp/canvas-luna-export-publication-review-followup-20260906.log`.
...
- A: 48 invalid segment rows (empty, dot, dotdot, slash, backslash, NUL, every U+0001–U+001F
  control, DEL, non-NFC combining form, case-insensitive Windows reserved names with extensions,
  and 256 UTF-8 bytes in ASCII and multibyte forms) plus valid 255-byte ASCII, valid 127 `é`
  plus `a` (255 UTF-8 bytes), NFC Unicode, and spaces. No normalization or sanitization is
  asserted.
...
- D: exact and case-fold duplicate bundle names, file/parent-directory conflicts in both orders,
  ...
  publication. Every invalid bundle case asserts that its destination is absent. Invalid names use
  `CANVAS_EXPORT_NAME_INVALID`; bundle collisions use `CANVAS_EXPORT_COLLISION`.
...
- F: two deterministic three-output late-failure cases use prepared exports plus the existing
  ...
  output absent. The replacement case renames the original to a retained path before creating the
  replacement, which preserves distinct inode identity; the replacement survives and its
  identity-change cleanup failure is returned. No sleeps or timing races are used.
