import { createDefaultSourceMetadata, isSharedGeneratedSourceMetadata } from './node-defaults'
import type { SceneNode } from './types'

export function markSourceFieldsEdited(node: SceneNode, changeKeys: string[]): void {
  if (changeKeys.length === 0) return
  if (isSharedGeneratedSourceMetadata(node.source)) node.source = createDefaultSourceMetadata()
