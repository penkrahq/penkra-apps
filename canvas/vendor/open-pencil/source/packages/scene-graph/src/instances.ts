...
function createMappedComponentClone(
  graph: SceneGraph,
  src: SceneNode,
  componentId: string,
  destParentId: string,
  ...
  const pencilAddress =
    typeof parent?.pencilAddress === 'string' && pencilNodeId
...
function cloneChildrenWithMapping(
  ...
  for (const childId of sourceParent.childIds) {
    ...
    const clone = createMappedComponentClone(graph, src, childId, destParentId, mode)
...
function syncChildren(
  ...
  for (const compChildId of compParent.childIds) {
    if (!instChildMap.has(compChildId)) {
      ...
      if (!src) continue
      const clone = createMappedComponentClone(graph, src, compChildId, instParentId)
      if (src.childIds.length > 0) {
