...
function applyIncludedStrokeBorders(yogaNode: YogaNode, node: SceneNode): void {
  if (!node.strokesIncludedInLayout) return
  const insideStrokes = node.strokes.filter(
    (stroke) => stroke.visible !== false && stroke.align === 'INSIDE'
  )
  if (insideStrokes.length === 0) return
  ...
  yogaNode.setBorder(Edge.All, Math.max(...insideStrokes.map((stroke) => stroke.weight)))
}
