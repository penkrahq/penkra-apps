...
import { setFigmaPaintBlendMode } from './blend'
import { applyPencilMeshFill, applyPencilShaderFill } from './pencil-effects'
...
export function paintFills(
  ...
  for (let index = 0; index < fills.length; index++) {
    ...
    setFigmaPaintBlendMode(r, r.fillPaint, fill.blendMode)
    draw(fill)
...
export function applyFill(
  ...
  if (fill.type === 'CUSTOM' && fill.pencilShader) {
    return applyPencilShaderFill(r, fill, node, graph)
  }

  if (fill.type === 'CUSTOM' && fill.pencilMesh) {
    return applyPencilMeshFill(r, fill, node)
  }
