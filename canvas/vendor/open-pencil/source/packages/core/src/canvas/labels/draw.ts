...
import { drawSingleFrameTitle } from './selection'
...
export function drawFrameTitles(
  ...
  const color = r.rulerTheme?.text ?? RULER_TEXT_COLOR
  for (const { node } of r.labelCache.getFrames(graph, r.worldViewport)) {
    if (!selectedIds.has(node.id)) {
      drawSingleFrameTitle(r, canvas, graph, node, {}, r.labelFont, color)
    }
  }
}
