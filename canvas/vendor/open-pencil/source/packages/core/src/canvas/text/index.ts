...
function styleRunDecorationValue(
  ck: CanvasKit,
  style: SceneNode['styleRuns'][number]['style'],
  fallback: SceneNode['textDecoration']
): number {
  if (style.underline !== undefined || style.strikethrough !== undefined) {
    return (style.underline ? ck.UnderlineDecoration : 0) |
      (style.strikethrough ? ck.LineThroughDecoration : 0)
  }
  return textDecorationValue(ck, style.textDecoration ?? fallback)
}
...
function pushStyleRun(
  ...
  builder.pushStyle(
    new ck.TextStyle({
      ...
      decoration: styleRunDecorationValue(ck, style, node.textDecoration),
