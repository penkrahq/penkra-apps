...
function applyCanvasIconDefinition(
  node: SceneNode,
  pen: PenNode,
  ctx: VarContext,
  ...
  if (definition.fontFamily) {
    // Applying ordinary text props first may already have measured a ligature and
    // expanded the node. Icons have an authored box, so use the resolved Pen size
    // captured before text measurement rather than that transient intrinsic size.
    ...
    // Width and height are readonly on the Pen scene-node adapter. Update the
    // backing graph after every text metric property has been applied.
    if (graph) graph.updateNode(node.id, { width, height })
...
function applyDescendantOverrides(
  ...
  for (const [origId, overrideData] of Object.entries(pen.descendants)) {
    ...
    if (clone) {
      ...
      const changed = applyOverrideProps(clone, overrideData, ctx, graph)
...
function fixTextWidths(graph: SceneGraph): void {
  for (const node of graph.getAllNodes()) {
    ...
    // Material Symbols are ligatures rendered inside an explicitly sized icon
    // box. Treating the ligature source text as ordinary narrow copy expands the
    // box to the length of names such as `progress_activity`.
    if (node.fontFamily.startsWith('Material Symbols ')) continue
