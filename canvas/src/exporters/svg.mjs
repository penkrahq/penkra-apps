...
function svgDefinitions(node, options) {
  ...
  if (fill?.type === "image") { const href = options.imageHref?.(fill.url); const imageMode = String(fill.mode ?? "fill").toLowerCase(); if (href) result.push(`<pattern id="fill-${escId(node.id)}" patternUnits="objectBoundingBox" width="1" height="1"><image href="${esc(href)}" width="1" height="1" preserveAspectRatio="${imageMode === "fit" ? "xMidYMid meet" : imageMode === "stretch" ? "none" : "xMidYMid slice"}"/></pattern>`); }
