...
test("aggregate capability gate uses explicit mobile fallbacks for unmeasured rows", () => {
  const entries = unverifiedCapabilityEntries();
  assert.deepEqual(entries, []);
  assert.equal(assertAllCapabilityTables(), true);
  for (const target of ["swift", "kotlin"]) {
    assert.equal(capabilityTableFor(target).properties["nodes.text"].verdict, "raster");
    assert.equal(capabilityTableFor(target).properties["properties.fill.solid"].verdict, "native");
  }
});
