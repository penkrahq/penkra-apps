...
test("collects every typed Canvas external resource exactly once", () => {
  const document = {
...
test("collects exact document fonts from Canvas-owned assets", () => {
...
test("resolves Canvas asset paths against their containing asset path", () => {
  assert.equal(resolvePencilResourcePath("../libraries/kit.json", "./icons/check.svg"), "../libraries/icons/check.svg");
  assert.equal(resolvePencilResourcePath("", "./assets/hero.png"), "assets/hero.png");
  const asset = { bytes: new Uint8Array([1]) };
  assert.equal(pencilResourceAsset(new Map([["../libraries/icons/check.svg", asset]]), "./icons/check.svg", "../libraries/kit.json"), asset);
});

test("assigns executable and Canvas asset MIME types explicitly", () => {
