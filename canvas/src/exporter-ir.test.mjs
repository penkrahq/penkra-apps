...
test("clipped raster scopes exclude descendants outside the authored frame", () => {
  const source = {
    module: "web",
    children: [{
      id: "route",
  ...
  const ir = buildExporterIR(source, { role: "route", frames: ["route"] });
  assert.equal(ir.rasters.length, 1);
  assert.equal(ir.rasters[0].id, "top");
  assert.deepEqual(ir.rasters[0].renderBounds, { minX: 0, minY: 0, maxX: 1280, maxY: 88 });
  assert.deepEqual(
    ir.rasters[0].variants.map(({ scale, pixelWidth, pixelHeight }) => ({ scale, pixelWidth, pixelHeight })),
    [
      { scale: 1, pixelWidth: 1280, pixelHeight: 88 },
      { scale: 2, pixelWidth: 2560, pixelHeight: 176 },
      { scale: 3, pixelWidth: 3840, pixelHeight: 264 },
    ],
  );
});
