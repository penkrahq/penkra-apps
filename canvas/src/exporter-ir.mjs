  ...
  const outputs = request.frames.map((frameId, index) => {
    ...
    return {
      ...
      root: {
        ...
        geometry: { x: 0, y: 0, localX: 0, localY: 0, w: graphNode.width, h: graphNode.height, rotation: graphNode.rotation ?? 0, ...(frame.flipX === true ? { flipX: true } : {}), ...(frame.flipY === true ? { flipY: true } : {}) },
  ...
  const visit = (node, parent = null, z = 0) => {
    ...
    if (node.id !== rootId || includeRoot) {
      ...
      result.push({
        ...
        paint: source.type === "icon" && format !== "html"
