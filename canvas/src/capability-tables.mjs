...
function mobileCandidateRows(platform, includesAlignItems) {
  ...
  return {
    ...
    ].filter((path) => !(platform === "Compose" && ["properties.gridTemplateColumns", "properties.layout", "properties.layoutPosition"].includes(path))).map((path) => [path, unverified(evidence)])),
