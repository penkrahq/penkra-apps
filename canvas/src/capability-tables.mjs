...
const EMISSION_SUPPORT = Object.freeze({
  ...
  pdf: table("select-at-export", [
    ...
    { "properties.cornerRadius": native("compatibility/pdf-rounded-rectangle-fidelity.test.mjs measures scalar and independent rounded corners rendered by Poppler against Canvas at 1x and 2x, excluding only a two-physical-pixel antialiasing boundary.") },
...
function mobileCandidateRows(platform, includesAlignItems) {
  ...
  return {
    ...
    ].map((path) => [path, unverified(evidence)])),
