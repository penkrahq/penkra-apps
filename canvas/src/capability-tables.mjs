...
function mobileCompletedRows(platform) {
  return {
    ...
    ...nativeRows(["properties.stroke", "properties.stroke.width", "properties.stroke.align", "properties.stroke.cap", "properties.stroke.join", "properties.stroke.dash", "properties.stroke.fill"], `${platform} retained vector-strokes, vector-dashes, dash-edge-cases, stroke-alignment and stroke-compositing captures measure solid width/color, butt/round/square caps, miter/round/bevel joins on multi-segment paths, normalized dash arrays and center/inside/outside clipping. Completion assertions separately bind exact width/cap/dash line output and the four-point solid inside-stroke color/bounds. Gradient stroke values are value-sensitively rasterized before this solid native profile.`),
    ...nativeRows(["properties.fill.gradient.linear", "properties.fill.gradient.radial", "properties.fill.gradient.angular", "properties.fill.gradient.radial.transformed"], `${platform} stable completion captures measure stop-color regions for rectangle linear, ellipse sweep/angular and nonuniform rotated radial gradients. Centroids verify the node-level linear flip/rotation and authored angular center/rotation; radial samples verify off-center 0.3/0.65, 0.7x1.25 scale and 24-degree transform. Vector-path gradient values are value-sensitively rasterized rather than entering this measured shape profile.`),
    ...nativeRows(["properties.fill.gradient.linear.transformed"], `${platform} rectangle transformed-linear emission is source-bound in src/exporters/mobile-capability-completion.test.mjs: center 0.3/0.6, size 0.7x0.8 and 20-degree paint rotation produce exact nondefault native endpoints in both appearance runtime branches. The retained phone capture measures the same rectangle-gradient primitive and node transform, but is not misreported as a paint-transform capture.`),
    ...
    ...nativeRows(["root.axes"], `${platform} completion runtime was captured in light/dark phone and light wide states. Exact root/probe colors verify appearance selection and the probe changes from 60 to 140 authored points at the 450-point viewport threshold; consecutive representative captures are hash-stable.`),
    ...nativeRows(["properties.modes", "properties.varies"], `${platform} node-local modes/varies paths are explicitly retained by semantic IR and source-bound to both native appearance branches in src/exporters/mobile-capability-completion.test.mjs. Runtime axis switching itself is device-measured under root.axes; these rows do not claim that the completion screenshot authored node-local mode metadata.`),
  };
}
